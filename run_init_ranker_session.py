...
from algos.rankutils import *
from algos.ranker import LambdaMART, DNN
...
def get_data(dataset, embed_dir):
    users, profiles, item_spars, item_denss, hist_dens, hist_spar, labels, list_lens = dataset
...
def train_mart(train_file, val_file, test_file, embed_dir, processed_dir, buckets,
    ...
    ranking_model_path = os.path.join(buckets, 'rank')
    ...
    rank(test_file, test_pred, os.path.join(ranking_model_path, parse.model_type + '.rankings.test'))
    ...
    rank(val_file, val_pred, os.path.join(ranking_model_path, parse.model_type + '.rankings.train'))
...
def ranker_parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='./avito/', help='the path of data')
    ...
    parser.add_argument('--setting_path', type=str, default='./configs/mart_setting.json', help='setting dir')
...
if __name__ == '__main__':
    ...
    data = load_file(os.path.join(processed_dir, 'data.vaild'))
    ...
    if parse.model_type == 'lambdaMART':
        train_mart(train_file, val_file, test_file, pt_dir,
              processed_dir, buckets, parse.tree_num, parse.lr, parse.tree_type)
