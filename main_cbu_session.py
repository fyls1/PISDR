...
def offline_session(sess, agent, dataset, args, epoch=0):
    ...
    if args.is_sequence or args.full_sequence:
        for _, eval_datas in CBUSessionKTimeActorDataInput(dataset, args.batch_size):
            batch_datas = get_aggregated_batch4cbu_session_k_seq(eval_datas)
            policy_action = agent.select_action(sess, batch_datas)
            ...
            batch_init_auc = calculate_bacth_prob_metrix(batch_datas["actions"]["single_score_label"], batch_datas["actions"]["single_ctr_label"])
            mean_ndcg, mean_map = evaluate_matrix(batch_preds=policy_action, batch_labels=batch_datas["actions"]["single_ctr_label"])
            init_ndcg, init_map = evaluate_matrix(batch_preds=batch_datas["actions"]["single_score_label"], batch_labels=batch_datas["actions"]["single_ctr_label"])
    ...
    to_show = "|EPOCH: {}| AUC:  {}| ORI_AUC: {}| NDCG: {}| MAP: {}| ORI_NDCG: {}| ORI_MAP: {}|".format(epoch, np.mean(auc_k), np.mean(init_auc_k), 
                                                                                                        np.mean(ndcg_k), np.mean(map_k),
                                                                                                        np.mean(init_ndcg_k), np.mean(init_map_k))
    print("-"*len(to_show))
    print(to_show)
    print("-"*len(to_show))
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if args.train_sl:
            ...
            for e in range(int(args.bc_steps)):
                train_losses = []
                for _, train_datas in CBUSessionSLDataInput(train_dateset, args.batch_size):
                    batch_datas = get_aggregated_batch4cbu_session_sl(train_datas)
                ...
                if e % 2 == 0:
                    ...
                    auc_12 = []
                    init_auc_12 = []
                    for _, eval_datas in CBUSessionSLDataInput(test_dateset, args.batch_size):
                        eval_batch_datas = get_aggregated_batch4cbu_session_sl(eval_datas)
                        ...
                        _init_auc_12 = calculate_bacth_prob_metrix(ori_probs, label)
                        auc_12.append(_auc_12)
                        init_auc_12.append(_init_auc_12)
                    mean_ndcg, mean_map = evaluate_matrix(batch_preds=probs, batch_labels=labels)
                    _show = "EPOCH: ", e, "; TRAIN LOSS:", np.mean(train_losses), "; EVAL LOSS:", np.mean(losses), "; AUC:", np.mean(auc_12), "; ORI_AUC:", np.mean(init_auc_12), "; NDCG:", mean_ndcg, "; MAP:", mean_map
                    # print("EPOCH: ", e, "; TRAIN LOSS:", np.mean(train_losses), "; EVAL LOSS:", np.mean(losses), "; AUC:", np.mean(auc_12), "; ORI_AUC:", np.mean(init_auc_12))
                    print("-"*len(_show))
                    print(_show)
                    print("-"*len(_show))
        ...
        for e in range(int(args.max_steps)):
            ...
            total_critic_loss = []
            if args.is_sequence or args.full_sequence:
                for _, train_datas in CBUSessionKTimeActorDataInput(ac_train_dataset, args.batch_size):
                    batch_datas = get_aggregated_batch4cbu_session_k_seq(train_datas)
...
def parse_args():
    ...
    parser.add_argument('--setting_path', type=str, default='./configs/multi_critic_cbu_session.json', help='setting dir')
...
if __name__ == '__main__':
    ...
    sl_train_reader = tf.python_io.TableReader(tables[0])
    total_train_num = sl_train_reader.get_row_count()
    if args.train_sl: 
        use_num = int(total_train_num * 1 * m)
    else:
        use_num = int(total_train_num * 1 * m)
    sl_train_dataset_set = sl_train_reader.read(use_num)
    print("Train SL dataset total length:", total_train_num, "Used num is:", use_num)
    sl_train_reader.close()

    sl_eval_reader = tf.python_io.TableReader(tables[2])
    total_eval_num = sl_eval_reader.get_row_count()
    if args.train_sl:  
        use_num = int(total_eval_num * 0.2 * m)
    else:
        use_num = int(total_eval_num * 0.2 * m)
    sl_eval_dataset_set = sl_eval_reader.read(use_num)
    print("Eval SL dataset total length:", total_eval_num, "Used num is:", use_num)
    sl_eval_reader.close()

    ac_train_reader = tf.python_io.TableReader(tables[1])
    ac_total_train_num = ac_train_reader.get_row_count()
    if not args.train_sl:
        use_num = int(ac_total_train_num * 1 * m)
    else:
        use_num = int(ac_total_train_num * 1 * m)
    print("Train Actor dataset total length:", ac_total_train_num, "Used num is:", use_num)
    ac_train_dataset_set = ac_train_reader.read(use_num)
    ac_train_reader.close()

    ac_eval_reader = tf.python_io.TableReader(tables[3])
    ac_total_eval_num = ac_eval_reader.get_row_count()
    if not args.train_sl:
        use_num = int(ac_total_eval_num * 0.2 * m)
    else:
        use_num = int(ac_total_eval_num * 0.2 * m)
    print("Eval Actor dataset total length:", ac_total_eval_num, "Used num is:", use_num)
    ac_eval_dataset_set = ac_eval_reader.read(use_num)
    ac_eval_reader.close()

    # ===== training =====
    run(sl_train_dataset_set, sl_eval_dataset_set, ac_train_dataset_set, ac_eval_dataset_set, args)