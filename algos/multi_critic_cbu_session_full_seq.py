...
import sys
from utils import *
from sl import SLCbuSessionActorOneHot,SLAvitoSessionActorOneHot
...
class Actor:
    ...
    def __init__(self, state_dim, num_actions, max_action, multi_num, name, args):
        ...
        with tf.variable_scope(self._name):
            ...
            self.seq_input_item = tf.placeholder(tf.float32, [None, args.max_action, 34], name='item_seq') #48+29
            self._build_item_transformer_output(args.max_action, args.d_model, args.max_action)
    ...
    def _build_item_transformer_output(self, max_item_num, d_model, seq):
        d_model = d_model
        num_layers = 2
        num_heads = 8
        dff = d_model * 2
        dropout_rate = 0.1
        self.seq_input_item_embed = tf.contrib.layers.fully_connected(
                inputs=self.seq_input_item,
                num_outputs=d_model,
                scope='layer_seq_item_input',
                activation_fn=tf.nn.relu,
                reuse=tf.AUTO_REUSE)
        self.item_decoder_out = decoder(self.seq_input_item_embed, max_item_num, num_layers, num_heads, [dff, d_model], dropout_rate, name="item_decoder")
        self._item_seq_state = tf.reshape(self.item_decoder_out, [-1, seq*d_model])
        self._state = tf.concat([self._state, self._item_seq_state], axis=-1)
    ...
    def predict(self, sess, seq_input, time, seq_item):
    ...
    def update(self, sess, seq_input, time, a, qvalue, weight, reward, seq_item):
...
class MULTI_CRITIC(object):
    def __init__(self, args):
        ...
        self.sl_actor = SLAvitoSessionActorOneHot(name="sl_actor_onehot", args=args)
    ...
    def select_action(self, sess, batch):
        ...
        # print("ac batch:", batch_size)

        ctr_rewards = np.reshape(rewards["ctr_rewards"], [batch_size, self.args.seq_length, 1])
        
        curr_seq_input, next_seq_input, seq_times, item_seq = self.generate_input(sess, states, next_states, actions, ctr_rewards, times, dones)
        # print("curr_seq_input shape:", len(curr_seq_input))
        return self.actor.predict(sess, curr_seq_input, seq_times[0], item_seq[0])
    ...
    def generate_input(self, sess, states, next_states, action, reward, times, dones):
        ...
        # print("generate_input batchsize:", batch_size)
        ...
        # output for item sequence input
        current_item_seq = np.zeros([batch_size, 12, 48+29])
        next_item_seq = np.zeros([batch_size, 12, 48+29])

        times = np.array(times).reshape([batch_size, seq_length, 1])

        for k in range(seq_length):
            _current_state, _item_embed = self.sl_actor.get_state_embed_item_embed(sess, user_spare, user_dense, item_spare[:, k], item_dense[:, k], hist_spare)
            _next_state, _next_item_embed = self.sl_actor.get_state_embed_item_embed(sess, next_user_spare, next_user_dense, next_item_spare[:, k], next_item_dense[:, k], next_hist_spare)
            ...
            if k == 0:
                current_item_seq = _item_embed
                next_item_seq = _next_item_embed
            else:
                for b in range(batch_size):
                    if times[b][k][0] != 0:
                        current_item_seq[b] = _item_embed[b]
                        next_item_seq[b] = _next_item_embed[b]
        ...
        return [current_action, current_reward, current_state], [next_action, next_reward, next_state], [current_time, next_time], [current_item_seq, next_item_seq]
    ...
    def train(self, sess, batch):
        ...
        target_q = self.target_critic.predict(sess, next_seq_input[-1], self.target_actor.predict(sess, next_seq_input, seq_times[1], item_seq[1]))
        ...
        # 更新 actor
        qvalue = self.critic.predict(sess, curr_seq_input[-1], self.actor.predict(sess, curr_seq_input, seq_times[0], item_seq[0]))
        actor_loss = self.actor.update(sess, curr_seq_input, seq_times[0], critic_actions, qvalue, weights, total_reward, item_seq[0])
