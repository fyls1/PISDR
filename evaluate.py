...
def offline_session(sess, agent, dataset, args, epoch=0):
    ...
    if args.is_sequence:
        ...
        for user_pvid, eval_datas in CBUSessionActorDataInput(dataset, args.batch_size):
            ...
            policy_action, qvalue = agent.select_action_critic(sess, batch_datas)
            Qvalue.extend(qvalue)
            probs.extend(policy_action)
            labels.extend(batch_datas["actions"]["ctr_label"])
            Dones.extend(batch_datas["dones"]["done"])
            if first:
                first = False
                print("prob:", probs)
                print("label:", labels)
    ...
    sum_reward, log_reward, log_times = total_reward_batch(probs, labels, Dones)
    ...
    _qvalue_show = "Reward Sum:", sum_reward, " ctr_value: ", np.mean(Qvalue[:,0]), " conv_value: ", np.mean(Qvalue[:,1]), " socre_value: ", np.mean(Qvalue[:,2])
    print("-"*len(to_show))
    print(to_show)
    print("-"*len(to_show))
    print(_qvalue_show)
...
def run(eval_dataset, args):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        Dones = []
        Qvalue = []
        for _, train_datas in CBUSessionActorDataInput(eval_dataset, args.batch_size):
            eval_batch_datas = get_aggregated_batch4cbu_session(train_datas)
            loss, prob, label = agent.sl_eval_rl(sess, eval_batch_datas)
            qvalue = agent.sl_critic_pre(sess, eval_batch_datas)
            Qvalue.extend(qvalue)
            losses.append(loss)
            probs.extend(prob)
            labels.extend(label)
            ori_probs = eval_batch_datas["actions"]["score_label"]
            OriProbs.extend(ori_probs)
            Dones.extend(eval_batch_datas["dones"]["done"])
        ...
        init_ndcg, init_map = evaluate_matrix(batch_preds=OriProbs, batch_labels=labels)
        _show = "SLModel: ", " EVAL LOSS:", np.mean(losses), "; AUC:", np.mean(auc_12), "; ORI_AUC:", np.mean(init_auc_12), "; NDCG:", mean_ndcg, "; MAP:", mean_map, "; ORI_NDCG:", init_ndcg, "; Ori_MAP:", init_map
        _qvalue_show = "Reward Sum:", sum_reward, " ctr_value: ", np.mean(Qvalue[:,0]), " conv_value: ", np.mean(Qvalue[:,1]), " socre_value: ", np.mean(Qvalue[:,2])
        print("-"*len(_show))
        print(_show)
        print("-"*len(_show))
        ...
        # evalue actor
        res_auc = offline_session(sess, agent, eval_dataset, args, 0)
...
if __name__ == '__main__':
    ...
    ac_train_reader = tf.python_io.TableReader(tables[0])
    ...
    ac_eval_reader = tf.python_io.TableReader(tables[1])
